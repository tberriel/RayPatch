...
class Attention(nn.Module):
    def __init__(self, dim, latent_dim = 768, heads=8, dim_head=64, dropout=0., selfatt=True, bias=False):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0., selfatt=True, latent_dim = 768, bias=False):
        super().__init__()
        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Attention(dim, latent_dim=latent_dim, heads=heads, dim_head=dim_head,
                                       dropout=dropout, selfatt=selfatt, bias=bias)),
