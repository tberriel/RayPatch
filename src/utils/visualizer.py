...
class Visualizer():
    ...
    def visualize_around(self, model, data, label, save_split, batch=None):
        ...
        with torch.no_grad():
            ...
            if self.reduced >1:
                start = int(self.reduced/2)
                input_rays_base = input_rays_base[:,start::self.reduced, start::self.reduced,:]
            ...
            output_img_path = os.path.join(self.out_dir, f'renders-{label}')
    ...
    def visualize_target_views(self, model, data, label, save_split, batch=0):
        ...
        with torch.no_grad():
            ...
            for i in range(num_target_images):
                img, extras = self.render_image(z, target_camera_pos[:,i], target_rays[:,i], model, **self.render_kwargs)
    ...
    def save_split(self, columns, batch=0):
        ...
        for i in range(len_batch):
            ...
            for j in range(len(columns)):
                img = Image.fromarray(np.uint8(columns[j][1][i]*255))
                if img.mode == 'RGBA':
                    img = img.convert('RGB')
