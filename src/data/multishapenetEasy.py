...
class MultishapenetEasyDataset(Dataset):
    ...
    def __getitem__(self, idx, noisy=True):
        ...
        all_rays = torch.tensor(np.stack(all_rays, 0).astype(np.float32))
        ...
        new_depths = []
        depths = self.transform_d(torch.tensor(depths))
        for i in range(self.num_views):
            new_depth = zs_to_depths(depths[i], all_rays[i], all_camera_pos[i])
            new_depths.append(new_depth)
        depths = torch.stack(new_depths, 0)

        imgs = self.transform_d(torch.tensor(imgs).permute(0,3,1,2))

        input_images = imgs[input_views]
        input_depth = depths[input_views]
        input_rays = all_rays[input_views]
        input_camera_pos = torch.tensor(all_camera_pos[input_views].astype(np.float32))

        target_images =imgs[target_views]
        target_depth = depths[target_views]
        target_rays = all_rays[target_views]
        target_camera_pos = torch.tensor(all_camera_pos[target_views].astype(np.float32))
        ...
        if self.target_reduced > 1:
            start = round(self.target_reduced/2)
            target_rays = target_rays[:,start::self.target_reduced, start::self.target_reduced,:]

        b, h, w, c = target_rays.shape
        target_shape = torch.tensor([target_images.shape[0], self.h, self.w, 3],dtype=torch.int32)
        ...
        target_camera_pos = target_camera_pos.unsqueeze(1).repeat((1, h * w, 1))
