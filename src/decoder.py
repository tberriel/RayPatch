...
class RayPredictor(nn.Module):
    def __init__(self, num_att_blocks=2, pos_start_octave=0, num_octaves=15, out_dims=3,  input_mlp=False, output_mlp=True, latent_dim=768):
        super().__init__()
        idim=num_octaves*6*2
        self.query_encoder = RayEncoder(pos_octaves=num_octaves, pos_start_octave=pos_start_octave,
                                        ray_octaves=num_octaves)
        self.transformer = Transformer(idim, depth=num_att_blocks, heads=12, dim_head=64,
                                       mlp_dim=1536, latent_dim = latent_dim, selfatt=False)
        if input_mlp:
...
class SRTDecoder(nn.Module):
    def __init__(self, num_att_blocks=2, pos_start_octave=0,num_octaves=15, out_dims = 3, out_mlp=True):
        super().__init__()
        ...
        self.ray_predictor = RayPredictor(num_att_blocks=num_att_blocks,
                                          ...
                                          latent_dim=768,
                                          input_mlp=True, output_mlp=not out_mlp
                                          )
...
class DeFiNeDecoder(nn.Module):
    def __init__(self, pos_start_octave=1,num_octaves=15, out_dims = 3):
        super().__init__()
        idim = num_octaves*6*2+6

        self.query_encoder = RayEncoder(pos_octaves=20, pos_start_octave=pos_start_octave,pos_end_octave=30, ray_octaves=10, ray_start_octave=1,ray_end_octave=30)
...
class FeatureDecoder(nn.Module):
    def __init__(self, conv_features=128, upsample = 3, out_dims=3, bnorm = True):
        super().__init__()
        ...
        for i in range(upsample):
            self.conv_blocks.append(nn.ModuleList([
                nn.Sequential(
                    ...
                    nn.BatchNorm2d(int(conv_features/2**(i+1))) if bnorm else nn.Identity(),
                    nn.LeakyReLU(0.2)
                ),
...
class RayPatchDecoder(nn.Module):
    def __init__(self, out_dims = 3, conv_features=128, h_in = 16, w_in = 16, h_out = None, w_out = None, upsample = 3, bnorm=True) -> None:
        super().__init__()
        self.h_in = h_in
        self.w_in = w_in
        ...
        self.conv_decoder = FeatureDecoder(conv_features, self.upsample, out_dims, bnorm=bnorm)
