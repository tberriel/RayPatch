...
from src.encoder import SRTEncoder, DeFiNeEncoder
from src.decoder import SRTDecoder, DeFiNeDecoder,  RayPatchDecoder
import time
...
class GenericModel(pl.LightningModule):
    ...
    def forward_pass(self, batch):        
        z = self.encoder(batch.get('input_images'), batch.get("input_camera_pos"), batch.get('input_rays'))
    ...
    def compute_loss(self, pred_pixels, extras, batch, append_key = "", sync_dist=False): 
        ...
        loss_mse = self.loss_colour(pred_pixels[vis_mask], batch.get("target_pixels")[vis_mask])

        self.log("mse"+append_key, loss_mse,sync_dist=sync_dist)
