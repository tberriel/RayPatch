...
from srt.model import SRT, DeFiNe
...
def load_dict(cfg, resolution, reduction, set_scannet_input=False):
    ...
    if cfg["model"]["decoder"] == "featurefield":
        for red in reduction:
            ...
            cfg_red['model']['decoder_kwargs']['h_in'] = int(resolution[0]/red)
            cfg_red['model']['decoder_kwargs']['w_in'] = int(resolution[1]/red)
            cfg_red['model']['decoder_kwargs']['h_out'] = resolution[0]
            cfg_red['model']['decoder_kwargs']['w_out'] = resolution[1]
            cfg_red['model']['decoder_kwargs']['upsample'] = int(math.log(red,2))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--frames', type=int, default=100, help='Frames processed to compute FPS.')
    ...
    if base_cfg["model"]["base"]=="define":
        model_dict["define"] = [DeFiNe, [[128,192],[240,320],[480,640],[960,1280]], [4,8,16,32]]
    elif base_cfg["model"]["base"]=="srt":
        model_dict["srt"] = [SRT, [[60,80], [120,160], [240,320]], [2,4,8]]
    else:
    ...
    for arch, item in model_dict.items():
        ...
        for res in item[1]:
            ...
            for decoder, cfg in decoder_dict.items():
                ...
                if args.time:
                    time_interactive, stdev = profile_model.time_interactive(data_sample, args.frames, 5, device)

                print("\nArch: {}; Decoder: {}; res: {}; GigaFLOPs: {}; render {} imgs: {} pm {}; FPS: ".format(arch, decoder, res, flops, args.frames, time_interactive, stdev, args.frames/time_interactive))
                if args.save:
                    with open(args.config_path+"/results.csv", 'a') as f:
